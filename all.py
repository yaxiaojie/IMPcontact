...
class IMP:
    ...
    def psiblast(self, filename, filepath):
        ...
        cmd = '/home/songjz671/blast/bin/psiblast -evalue 10 -num_iterations 3 -db /home/songjz671/blast/db/uniprot -query ' + filepath + '.fasta -outfmt 0 -out ./psiblastout/' + pdbid + '.fm0 -out_ascii_pssm ./pssm/' + pdbid + '.pssm -num_alignments 1500 -num_threads 48'
        os.system(cmd)
    """
    ##############   HMMTOP   #################
    def hmmtop(self, filename, filepath):
        ...
        cmd = 'hmmtop -if=' + filepath + ' -pl -of=/home/jiayj267/IMP/nr/hmmtop/' + pdbid + '.hmm'
        os.system(cmd)
    ...
    def cover_hmmtop(self, filename):
        pdbid = filename.split('.')[0]
        firstline = '>' + pdbid + '\n'
        hmmtoppath = '/home/jiayj267/IMP/nr/hmmtop/' + pdbid + '.hmm'
        with open(hmmtoppath, 'r') as f:
            ...
            while i <= len(lines)-2:
                ...
                seq = seq+pre2
                i = i+3
            parsedfile = open('/home/jiayj267/IMP/nr/topology/'+pdbid+'.txt', 'w')
    ...
    def mview(self, filename):
        pdbid = filename.split('.')[0]
        cmd = '/home/ThirdPartTools/mview-1.61/bin/mview -in blast -out fasta /home/RaidDisk/IMP/runOver/fm0/' + pdbid + '.fm0 > /home/jiayj267/IMP/nr/mview/' + pdbid + '.aln'
        os.system(cmd)
    ...
    def convertcovarianceinput(self, filename):
        pdbid = filename.split('.')[0]
        filepath = '/home/jiayj267/IMP/nr/mview/'+pdbid + '.aln'
        ...
        with open(filepath, 'r') as f:
            ...
            count = len(mview)
            i = 2
            inputfilepath = '/home/jiayj267/IMP/nr/covariance/input/' + pdbid + '.txt'
    ...
    def Covariance(self, filename):
        pdbid = filename.split('.')[0]
        cmd = 'java covariance.algorithms.ELSCCovariance /home/jiayj267/IMP/nr/covariance/input/' + pdbid + '.txt /home/jiayj267/IMP/nr/covariance/output/ELSC/' + pdbid + '.txt'
        os.system(cmd)

    ##############   ZScore  #################
    def zscore(self, filename):
        pdbid = filename.split('.')[0]
        filepath = '/home/jiayj267/IMP/nr/covariance/output/ELSC/' + pdbid + '.txt'
        meannum, stdnum = self.getCalculationParameters(filepath)
        outfile = '/home/jiayj267/IMP/nr/covariance/zscore/ELSC/' + pdbid + '_zscore.txt'
    ...
    def dealTopologyFile(self, filename):
        pdbid = filename.split('.')[0]
        filepath = '/home/jiayj267/IMP/nr/topology/' + pdbid + '.txt'
        residuePosition = {"i": 1, "m": 0, "o": -1}
        path = '/home/jiayj267/IMP/nr/feature/' + pdbid + '.txt'
        with open(path, 'r') as f2:
            ...
            with open(filepath, 'r') as f:
                ...
                for i in range(len(topologyLines[1])):
                    ...
                    writefile = open('/home/jiayj267/IMP/nr/featurenew/'+pdbid+'.txt', "a")
    ...
    def getRelativeFeature(self, filename):
        ...
        with open(filepath, 'r')as f:
            topologyLines = f.readlines()
            path = '/home/jiayj267/IMP/nr/featurenew/' + pdbid + '.txt'
            with open(path, 'r') as f2:
                filelines = f2.readlines()
    ...
    def predictDataGenerate(self, filename):
        pdbid = filename.split('.')[0]
        with open('/home/jiayj267/IMP/nr/covariance/zscore/ELSC/' + pdbid + '_zscore.txt', 'r')as f:
            covarianceLines = f.readlines()
            covariancecount = len(covarianceLines)
            featurefile = '/home/jiayj267/IMP/nr/featurenew/' + pdbid + '.txt'
            with open(featurefile, 'r')as f2:
                featureLines = f2.readlines()
                featurecount = len(featureLines)
                outfile = '/home/jiayj267/IMP/nr/slidwindow/ELSC/predict/' + pdbid + '.txt'
                writefile = open(outfile, "w+")
                orderpath = '/home/jiayj267/IMP/nr/slidwindow/ELSC/order/' + pdbid + '.txt'
...
def main():
    ...
    #############   遍历文件夹中的所有文件进行下面的操作   #################
    rootdir = '/home/jiayj267/IMP/Data/pdbsequence/training/'
    for dirpath, dirnames, filenames in os.walk(rootdir):
        for filename in filenames:
            print(filename)
            filepath = os.path.join(dirpath, filename)
